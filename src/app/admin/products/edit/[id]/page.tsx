...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      // Hydrate colours from server.
      const serverColours = Array.isArray(prod.colours) ? prod.colours : [];
      setColours(serverColours.map((c: any) => ({ name: c.name, hex_code: c.hex_code })));

      // Hydrate scale_variations from server.
      const sv = prod.scale_variations;
      if (sv?.enabled && Array.isArray(sv.scales) && sv.scales.length > 0) {
        setScaleEnabled(true);
        setScales(sv.scales.map((s: any) => ({ label: s.label, price: Number(s.price) })));
      }
    } catch (err) {
